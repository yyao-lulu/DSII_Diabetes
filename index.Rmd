...
# create training set
set.seed(1)
nrow_sample = sample(1:nrow(diabetes_tidy), 10000)
```
...
readmit = diabetes_tidy[nrow_sample,]$readmitted
train = as.data.frame(diabetes_tidy[nrow_sample, c(6:12, 16)])
test = diabetes_tidy[-nrow_sample, c(6:12, 16)]
...
library(caret)
confusionMatrix(fit_knn, diabetes_tidy[-nrow_sample,]$readmitted)
```
...
plot(fit_tree)
pred_tree = predict(fit_tree, newdata = diabetes_tidy[-nrow_sample,])
confusionMatrix(pred_tree, diabetes_tidy[-nrow_sample,]$readmitted)
```
...
fit_rf = randomForest(readmitted~., data = diabetes_rf, subset = nrow_sample, mtry = 6, importance = TRUE)
importance(fit_rf)
pred_rf = predict(fit_rf, newdata=diabetes_tidy[-nrow_sample,],type="response")
# confusionMatrix(pred_rf, diabetes_tidy[-nrow_sample,]$readmitted)
```
...
library(caret)
# model construction
svm.model = svm(readmitted~.,data=diabetes_tidy[nrow_sample,c(-1,-2)],type="C-classification",cost=10,gamma=0.01)
# training error
svm.pred = predict(svm.model,newdata=diabetes_tidy[nrow_sample,c(-1,-2)])
confusionMatrix(svm.pred,diabetes_tidy[nrow_sample,]$readmitted)
# test error
svm.pred.test = predict(svm.model,newdata=diabetes_tidy[-nrow_sample,c(-1,-2)])
confusionMatrix(svm.pred.test,diabetes_tidy[-nrow_sample,]$readmitted)

```

#### Logistic Regression
```{r logistic}
logit.fit = glm(readmitted~.-encounter_id-patient_nbr,data=diabetes_tidy,subset=-nrow_sample,family="binomial")
# contrasts(diabetes_tidy$readmitted)
test.pred.prob = predict(logit.fit,newdata=diabetes_tidy[nrow_sample,],type="response")
# confusionMatrix
```
