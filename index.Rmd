...
importance(fit_rf)
pred_rf = predict(fit_rf, newdata = diabetes_tidy[-train_sample,])
confusionMatrix(pred_rf, diabetes_tidy[-train_sample,]$readmitted)
```
...
library(caret)
# model construction
svm.model = svm(readmitted~.,data=diabetes_tidy[train_sample,c(-1,-2)],type="C-classification",cost=10,gamma=0.01)
# training error
svm.pred = predict(svm.model,newdata=diabetes_tidy[train_sample,c(-1,-2)])
confusionMatrix(svm.pred,diabetes_tidy[train_sample,]$readmitted)
# test error
svm.pred.test = predict(svm.model,newdata=diabetes_tidy[-train_sample,c(-1,-2)])
confusionMatrix(svm.pred.test,diabetes_tidy[-train_sample,]$readmitted)
